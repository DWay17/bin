...
REGEX_FILEMX21="7z|apk|al|asc|bak|bash_.*|bashrc|bz2|cfg|classpath|conf|config|crt|crx|csv|dat|data|db|diff|docx?|eps|eml|gpg"
...
for dir in `find . -maxdepth $MAXDEPTHD -type d` ; do
	...
	find . -maxdepth $MAXDEPTH -mindepth 1 -type f -regextype posix-extended -regex '.*\.(sh|bash|cmd|bat|exe|com|pl|php|py)' -exec chmod -c u+rwx {} \;
	echo "directories +x"
