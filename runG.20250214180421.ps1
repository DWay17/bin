...
foreach ($StartMenuFolder in $StartMenuFolders) {
    ...
    if ($lnkFiles) {
        Write-Host "    .lnk-Dateien gefunden in Ordner '$StartMenuFolder':" # Ausgabe hinzugefügt
        foreach ($lnkFile in $lnkFiles) {
            ...
            try {
                ...
                if ($Shell -eq $null) { # Prüfe, ob $Shell null ist
                    Write-Warning "Fehler: WScript.Shell COM-Objekt konnte nicht erstellt werden! Datei: '$($lnkFile.FullName)'" # Dateiinfo hinzugefügt
