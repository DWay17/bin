...
find . -maxdepth $MAXDEPTH -type f -regextype posix-extended -regex '.*\.(asc|bak|cfg|csv|dat|db|ini|js|json|kdbx?|lock|log|mail|md|pdf|properties|rdf|sql|sqlite|txt|xlsx?|xml|xsl|xpi|zip|gitignore|project)' -exec chmod -c a-x {} \; 
